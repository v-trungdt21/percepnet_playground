...
def rapt(wavfile_path, **kwargs):
    ...
    return freq_estimate, original_audio[0]


def rapt_sample(sample, sample_rate, **kwargs):
    """
    F0 estimator inspired by RAPT algorithm to determine vocal
    pitch of an audio sample.
    """
    ...
    # TODO: Flesh out docstring, describe args, expected vals in kwargs
    original_audio = (sample_rate, sample)
    ...
    # Dynamic programming - determine voicing state at each period candidate
    # print("nccf_results", nccf_results[0])
    freq_estimate = _get_freq_estimate(nccf_results[0], param,
                                       original_audio[0])
    corrs = _select_max_correlation_for_frame(nccf_results[0][0])
...
def _downsample_audio(original_audio, downsampling_rate):
    ...
    downsampled_audio = signal.resample(original_audio[1], int(number_of_samples))
...
def _one_pass_nccf(audio, raptparam):
    ...
    for i in range(0, params[1].max_frame_count):
...
def _first_pass_nccf(audio, raptparam):
    ...
    # we do not run into issues with using range to iterate thru each frame, i
...
def _select_candidates(nccf_results, params, sample_rate):
    ...
    final_candidates = _get_next_cands(0, frame_candidates, nccf_results,
                                       params, sample_rate)
...
def _get_next_cands(frame_idx, prev_candidates, nccf_results, params,
                    sample_rate):
    # print("nccf_results hearrrrrrrrr", nccf_results)
